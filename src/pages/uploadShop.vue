        ...
        <el-form ref="form" :model="form" label-width="80px">
            ...
            <el-form-item label="logo上传" >
                <label class="shop__logo-area">
                    <input type="file" @change="setLogo" style="width: 0;height: 0;opacity: 0" >
    ...
    export default {
        ...
        methods: {
            ...
            handleShopInfo(data) {
                ...
                this.logo = data.logo ? (window.config.DOMAIN + data.logo): ''
                this.form.shows = data.album ? JSON.parse( data.album ): []
                this.shows = data.album ? JSON.parse( data.album ).map(function (item) {
...
//                this.map.enableScrollWheelZoom(true);     //开启鼠标滚轮缩放
                ...
                if( this.form.lon && this.form.lat ){
                    _this.point = new BMap.Point(this.form.lon, this.form.lat)
...
//                            _this.logo = img
                            var formData = new FormData();
                             ...
                             upFiles(formData,'?type=company').then(function (res) {
                                 if(res.data.code === window.config.STATE_OK){
                                     _this.$message({
                                         message: '上传成功',
                                         type: 'success'
                                     ...
                                     _this.logo = window.config.DOMAIN + _this.form.logo
                                 }
                             })
                        }
                    })
...
//                            _this.shows.push(img)
                            ...
                            upFiles(formData,'?type=company').then(function (res) {
                                if(res.data.code === window.config.STATE_OK){
                                    ...
                                    _this.shows.push(window.config.DOMAIN + res.data.data[0].saveFile)
                                }
                            })
                        }
                    })
